...
- `--no-gbif-family`: skip live GBIF calls while still using cached family values
...
Use [backfill_version2_es.py](backfill_version2_es.py) to add version2 field mappings and backfill existing live records without reloading every source file. The script copies old field names into the version2 names, renames `SeasonWatch India` to `SeasonWatch (India)`, derives `mappedTraitsUrns` from `data/traits.csv`, and fills `gbifFamily` from GBIF using a reusable cache.
...
The default run is read-only. Use `--apply` to write updates to Elasticsearch.
