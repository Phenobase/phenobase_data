...
The trait reasoning step comes first. The loader depends on `data/traits.csv` to resolve each incoming `traitUrn` or source `trait_urn` into the current canonical `trait` label, derived `mappedTraits`, and derived `mappedTraitsUrns` used later for indexing and querying.
...
5. Confirm the loader inputs are in place:
   ...
   - `data/traits.csv` provides the PPO ID-to-`trait`, `mappedTraits`, and `mappedTraitsUrns` expansion generated by the reasoning step
   - `transform.yaml` is optional and only applies to the dataset directory being loaded

6. Before validation or loading, extend the reusable GBIF family cache for the source CSVs you are about to load. This keeps normal loader runs from blocking on live GBIF requests:

```bash
python3 prepare_gbif_family_cache.py \
  --names-from-csv <data_dir> \
  --gbif-workers 16
```

7. Run a dry ingestion pass in cache-only mode:

```bash
python3 loader.py --mode=<machine|in_situ|herbarium> --test --no-drop-existing --gbif-cache-only <data_dir>
```

8. Review `loading_errors.csv`. Common failures are:

   - missing `annotationID`
   - duplicate `annotationID` within an input file
   - empty `trait` and `traitUrn`/source `trait_urn`
   - `traitUrn` or source `trait_urn` values not found in `data/traits.csv`
   - legacy `trait` values not found in `data/traits.csv` when no trait ID is provided
   - strict-mode coercion failures for typed fields

9. Fix the source files or the dataset-local `transform.yaml`, then rerun test mode until the release is acceptable.
10. Run the production reload:

```bash
python3 loader.py --mode=<machine|in_situ|herbarium> --drop-source-records --gbif-cache-only <data_dir>
```
...
11. If you are only appending new records or intentionally upserting by existing
`annotationID` values without deleting stale source records, use:

```bash
python3 loader.py --mode=<machine|in_situ|herbarium> --no-drop-existing --gbif-cache-only <data_dir>
```

12. If you are rebuilding the index from scratch rather than appending or updating, use:

```bash
python3 loader.py --mode=<machine|in_situ|herbarium> --drop-existing --gbif-cache-only <data_dir>
```

13. If the live index needs `decadeStart`, run:
...
```bash
python3 loader.py --mode=machine --drop-source-records --gbif-cache-only data/annotations.07.25.2025/ --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only data/npn.1956.01.01-2025.08.31/ --batch-size 5000 --progress-every 50000
```
...
For normal source loads, pre-resolve names into that cache before running `loader.py`. This avoids slow per-row live GBIF calls during validation and indexing.

```bash
python3 prepare_gbif_family_cache.py \
  --names-from-csv downloads/npn/ingest \
  --names-from-csv downloads/phenoObs/ingest \
  --names-from-csv downloads/budburst/ingest \
  --names-from-csv downloads/seasonwatchindia/ingest \
  --gbif-workers 16 \
  --cache-write-every 500
```

Then run the loader with `--gbif-cache-only`:

```bash
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/npn/ingest
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/npn/ingest
```

The loader uses cached family values even when live GBIF calls are disabled. Cache misses remain blank, so the cache-prep step should be run before production loads. `--no-gbif-family` is kept as a backward-compatible alias for the same cache-only behavior.
...
Refresh the GBIF family cache from the prepared ingest CSVs before validation or loading:
...
python3 prepare_gbif_family_cache.py \
  ...
  --names-from-csv downloads/budburst/ingest \
  --names-from-csv downloads/seasonwatchindia/ingest \
  --names-from-csv downloads/iNaturalist/ingest \
  --gbif-workers 16 \
  --cache-write-every 500
```
...
```bash
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/npn/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/phenoObs/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/budburst/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/seasonwatchindia/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/iNaturalist/ingest --batch-size 5000 --progress-every 50000
```
...
```bash
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/npn/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/phenoObs/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/budburst/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/seasonwatchindia/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/iNaturalist/ingest --batch-size 5000 --progress-every 50000
```
...
```bash
python3 prepare_gbif_family_cache.py \
  --names-from-csv downloads/herbarium/ingest \
  --gbif-workers 16 \
  --cache-write-every 500
python3 loader.py --mode=herbarium --test --no-drop-existing --gbif-cache-only downloads/herbarium/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=herbarium --drop-source-records --gbif-cache-only downloads/herbarium/ingest --batch-size 5000 --progress-every 50000
```

In herbarium mode, records with only year precision and no month-bearing `date` are rejected during loading.
...
cd ../..
python3 prepare_gbif_family_cache.py --names-from-csv downloads/phenoObs/ingest --gbif-workers 16
python3 loader.py --mode=in_situ --test --no-drop-existing --gbif-cache-only downloads/phenoObs/ingest --batch-size 5000 --progress-every 50000
python3 loader.py --mode=in_situ --drop-source-records --gbif-cache-only downloads/phenoObs/ingest --batch-size 5000 --progress-every 50000
```
...
SeasonWatch India source preparation writes `observedMetadataUrl`, which the loader normalizes to `sourceRecordUrl`, as direct GBIF occurrence
...
- `data/traits.csv` for `traitUrn`/source `trait_urn` to `trait`, `mappedTraits`, and `mappedTraitsUrns` expansion
...
- `trait_mappings`: exact mapping from lowercase raw `trait` text to canonical trait labels. Prefer trait IDs in source prep, and treat this as a compatibility tool.
...
- `trait_mappings` is only applied to the `trait` field after field transforms run; it does not replace trait IDs
...
Build the all-datasets example package with 20 records per `dataSource` and exact live source counts:
...
If direct ES access is blocked, use the Phenobase query proxy instead:
...
Then run the ES backfill cache-only:

```bash
python3 backfill_version2_es.py \
  --host 149.165.170.158 \
  --port 8081 \
  --scheme http \
  --apply \
